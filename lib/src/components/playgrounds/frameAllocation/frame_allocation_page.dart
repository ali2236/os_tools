...
import 'package:os_aligator_ir/src/components/playgrounds/frameAllocation/allocation_timeline.component.dart';
...
@Component(
  ...
  <playground-back-button></playground-back-button>
      ...
      <material-tab-panel [activeTabIndex]="tabIndex">
...
class FrameAllocationPage extends PlaygroundPage {
  ...
  List<int> parseInput() {
    return (pageInput ?? pagePlaceholder).split(' ').map(int.tryParse).toList();
  }
  ...
  String tabTitle(String algorithmName){
    return '$algorithmName(${result[algorithmName].pageFaults})';
  }

  void calculate() {
    var input = parseInput();
    var timelines = algorithms.map((e) => e.calculate(input, frameCount)).toList();
